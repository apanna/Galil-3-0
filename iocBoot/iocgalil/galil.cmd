...
dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_motors.substitutions")

#Load DMC controller features (eg.  Limit switch type, home switch type, output compare, message consoles)
dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_dmc_ctrl.substitutions")

#Load RIO controller features (eg.  Model, IP address, message consoles)
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_rio_ctrl.substitutions")

#Load extra features for real axis/motors (eg. Motor type, encoder type)
dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_motor_extras.substitutions")

#Load kinematics for CS axis/motors (eg. Forward and reverse kinematics, kinematic variables)
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_csmotor_kinematics.substitutions")

#Load coordinate system features (eg. Coordinate system S and T status, motor list, segments processed, moving status)
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_coordinate_systems.substitutions")

#Load digital IO databases
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_digital_ports.substitutions")

#Load analog IO databases
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_analog_ports.substitutions")

#Load user defined functions
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_userdef_records.substitutions")

#Load user defined array support
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_user_array.substitutions")

#Load profiles
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_profileMoveController.substitutions")
#dbLoadTemplate("$(TOP)/iocBoot/$(IOC)/galil_profileMoveAxis.substitutions")
...
GalilCreateController("Galil", "10.0.0.201", 8)
